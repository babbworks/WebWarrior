...
async function showSection(name, { noScroll = false } = {}) {
  ...
  // Track active section on body for CSS targeting (e.g. river mode glass panels)
  document.body.className = document.body.className
    .replace(/\bsection-\S+/g, '').trim();
...
function activateJournalTheme(theme) {
  ...
  const twainEls = ['twain-scratch-col', 'twain-save-bar', 'twain-task-bar', 'twain-entries-bar', 'btn-twain-new-entry'];
  ...
  if (theme === 'twain') {
    ...
    if (proj) proj.placeholder = 'section';
    ...
    // Load recent sections/tags + hidden list for ink panel
...
function clearTwainJournalState() {
  // Scratchpad
  const scratch = document.getElementById('twain-scratch');
  if (scratch) scratch.value = '';
  localStorage.removeItem('ww_twain_scratch');
  // Sections
  twainRecentSections = [];
  twainHiddenSections = new Set();
  localStorage.removeItem('ww_twain_sections');
  localStorage.removeItem('ww_twain_sections_hidden');
  // Tags
  twainRecentTags = [];
  localStorage.removeItem('ww_twain_tags');
  // Meta inputs
  const proj = document.getElementById('journal-project-input');
  if (proj) proj.value = '';
...
function mergeTwainSectionsFromEntries(entries) {
  // Pull unique non-empty project values from all loaded entries
  const fromEntries = [...new Set(
    entries.map(e => (e.project || '').trim()).filter(Boolean)
  )];
  // Merge: entry-derived first (preserving order), then manually added, skip hidden
...
function wireJournalSection() {
  ...
  document.getElementById('journal-name-select')?.addEventListener('change', (e) => {
    ...
    if (journalTheme === 'twain') clearTwainJournalState();
    loadJournal();
  });
  ...
  document.getElementById('btn-twain-new-entry')?.addEventListener('click', async () => {
    const editingId = form?.dataset.editingId ? parseInt(form.dataset.editingId, 10) : null;
    if (editingId) {
      // Save current edits then reset to blank
      const body = textarea?.value.trim();
      if (body) {
        const fd = new FormData(form);
        await Journal.updateEntry(activeProfile, editingId, {
    ...
    // Clear writing area for a new entry
    if (textarea) textarea.value = '';
    const proj = document.getElementById('journal-project-input');
    const tags = document.getElementById('journal-tags-input');
    const prio = document.getElementById('journal-priority-select');
    if (proj) proj.value = '';
    if (tags) tags.value = '';
    if (prio) prio.value = '';
    textarea?.focus();
  ...
  // Entries bar: toggle opens drawer; clicking the info text focuses search
  document.getElementById('btn-twain-entries-toggle')?.addEventListener('click', () => {
  ...
  // Entries search filters the drawer in real time
  document.getElementById('twain-entries-search')?.addEventListener('input', (e) => {
    const q = e.target.value.toLowerCase();
    document.querySelectorAll('#twain-entries-drawer .journal-entry').forEach(el => {
      const text = el.textContent.toLowerCase();
      el.style.display = text.includes(q) ? '' : 'none';
    });
    document.querySelectorAll('#twain-entries-drawer .journal-date-group').forEach(grp => {
      const anyVisible = [...grp.querySelectorAll('.journal-entry')].some(e => e.style.display !== 'none');
      grp.style.display = anyVisible ? '' : 'none';
    });
  });

  // Click a past entry in the Twain drawer → load it into the writing textarea
  document.getElementById('twain-entries-drawer')?.addEventListener('click', async (e) => {
    if (e.target.closest('[data-action]') || e.target.closest('.jrnl-action-pill')) return;
    const entryEl = e.target.closest('.journal-entry[data-id]');
    if (!entryEl) return;
    const id = parseInt(entryEl.dataset.id, 10);
    const entry = await Journal.getEntry(activeProfile, id);
    if (!entry) return;
    const ta = document.getElementById('journal-entry-textarea');
    if (ta) {
      ta.value = entry.body || '';
    ...
    // Store id for update-on-save (attach to form)
    const fm = document.getElementById('add-journal-form');
    if (fm) fm.dataset.editingId = id;
    // Scroll drawer entry into view
    entryEl.scrollIntoView({ block: 'nearest' });
  });

  // Collapse scratch pad
  ...
  document.getElementById('btn-scratch-collapse')?.addEventListener('click', toggleScratchCollapse);
  document.getElementById('btn-twain-expand')?.addEventListener('click', toggleScratchCollapse);
  ...
  // Section field: Enter adds to ink panel
  ...
  // Tags field: Enter adds tag to ink panel
  ...
  // River mode button (save bar) — stays in sync with mode select
  ...
  // Task drawer
  function closeTwainTaskDrawer() {
    document.getElementById('twain-task-bar')?.classList.remove('open');
    document.getElementById('twain-task-drawer')?.classList.add('hidden');
    document.getElementById('twain-task-input') && (document.getElementById('twain-task-input').value = '');
    document.getElementById('twain-task-project') && (document.getElementById('twain-task-project').value = '');
    document.getElementById('twain-task-priority') && (document.getElementById('twain-task-priority').value = '');
  }

  document.getElementById('btn-twain-task-tab')?.addEventListener('click', () => {
    const bar    = document.getElementById('twain-task-bar');
    const drawer = document.getElementById('twain-task-drawer');
    const input  = document.getElementById('twain-task-input');
    const isOpen = bar?.classList.contains('open');
    bar?.classList.toggle('open', !isOpen);
    drawer?.classList.toggle('hidden', isOpen);
    if (!isOpen) setTimeout(() => input?.focus(), 30);
  });

  async function submitTwainTask() {
    const input    = document.getElementById('twain-task-input');
    const projEl   = document.getElementById('twain-task-project');
    const prioEl   = document.getElementById('twain-task-priority');
    ...
    showToast(`Task added: ${desc}`);
    closeTwainTaskDrawer();
  }

  document.getElementById('btn-twain-task-add')?.addEventListener('click', submitTwainTask);

  document.getElementById('twain-task-input')?.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') { closeTwainTaskDrawer(); return; }
    if (e.key === 'Enter') { e.preventDefault(); submitTwainTask(); }
  });

  document.getElementById('twain-task-project')?.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') { closeTwainTaskDrawer(); return; }
    if (e.key === 'Enter') { e.preventDefault(); submitTwainTask(); }
  });
  ...
  if (journalTheme === 'twain') {
    ...
    updateThemeModeSelect('twain', false); // don't reset — river may have been active
  } else {
...
async function loadLedger() {
  ...
  const [txns, balances] = await Promise.all([
    Ledger.getTransactions(activeProfile, { ledger: activeLedger, search: filterText }),
...
function wireLedgerSection() {
  ...
  document.getElementById('ledger-search')?.addEventListener('input', async (e) => {
    filterText = e.target.value;
    const txns = await Ledger.getTransactions(activeProfile, { ledger: activeLedger, search: filterText });
    Render.renderTransactions(txns);
  });
