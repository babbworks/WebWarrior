...
async function loadJournal() {
  ...
  if (journalTheme === 'twain') updateTwainEntriesInfo(entries);
}
...
function activateJournalTheme(theme) {
  ...
  const twainEls = ['twain-scratch-col', 'twain-save-bar', 'twain-entries-bar'];
...
function updateTwainContextPanel() {
  if (journalTheme !== 'twain') return;
  const panel = document.getElementById('twain-context-panel');
  if (!panel) return;
  ...
  const schips = document.getElementById('twain-section-chips');
  if (schips) {
    schips.innerHTML = twainRecentSections.map(s =>
      `<button class="twain-ctx-chip${s === activeProj ? ' active' : ''}" data-ctx-section="${esc(s)}">${esc(s)}</button>`
    ).join('');
    schips.classList.toggle('hidden', twainSectionsCollapsed);
  }
  const tchips = document.getElementById('twain-tag-chips');
  if (tchips) {
    tchips.innerHTML = twainRecentTags.map(t =>
      `<button class="twain-ctx-chip${activeTags.includes(t) ? ' active' : ''}" data-ctx-tag="${esc(t)}">${esc(t)}</button>`
    ).join('');
    tchips.classList.toggle('hidden', twainTagsCollapsed);
  }
  const hasContent = twainRecentSections.length > 0 || twainRecentTags.length > 0;
...
function wireJournalSection() {
  ...
  document.getElementById('journal-theme-select')?.addEventListener('change', (e) => {
    activateJournalTheme(e.target.value);
    if (e.target.value === 'twain') loadJournal();
  });
  ...
  // @sections field: Enter adds chip to context panel
  ...
  document.getElementById('journal-project-input')?.addEventListener('input', () => {
    if (journalTheme === 'twain') updateTwainContextPanel();
  });
  ...
  // Context panel clicks
  document.getElementById('twain-context-panel')?.addEventListener('click', (e) => {
    ...
    if (e.target.closest('#btn-collapse-sections')) {
      ...
      document.getElementById('twain-section-chips')?.classList.toggle('hidden', twainSectionsCollapsed);
    ...
    if (e.target.closest('#btn-collapse-tags')) {
      ...
      document.getElementById('twain-tag-chips')?.classList.toggle('hidden', twainTagsCollapsed);
  ...
  if (journalTheme === 'twain') activateJournalTheme('twain');
}
